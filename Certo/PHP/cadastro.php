...
if ($conexao->connect_errno) {
    ...
    if($senha == $codi){
        $SQL = 'INSERT INTO `usuario_` (`nome`,`tipo`,`senha`,`cod_grupo`,`cargo`,`cod_prof`) VALUES ("' . $nome . '","Professor","' . $senha . '","' . $codg . '","' . $cargo . '","' . $codi . '");';
        $resultado = $conexao->query($SQL);
        $conexao->close();
        header("Location: ../HTML/index.html");
    }else{

    $SQL = 'INSERT INTO `usuario_` (`nome`,`tipo`,`senha`,`cod_grupo`,`cargo`,`cod_prof`) VALUES ("' . $nome . '","Aluno","' . $senha . '","' . $codg . '","' . $cargo . '","' . $codi . '");';
    $resultado = $conexao->query($SQL);
    $conexao->close();
    header("Location: ../HTML/index.html");
}}
